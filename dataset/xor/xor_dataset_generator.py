...
def generate_train_set():
    # 训练集
    training_set = []
    node_total_x = np.random.rand(train_size) * (x_max - x_min) - (x_split - x_min)
    node_total_y = np.random.rand(train_size) * (y_max - y_min) - (y_split - y_min)
    # 构造训练集
    for i in range(0, train_size):
        if node_total_x[i] * node_total_y[i] > 0:
            ...
            tag = "B"
        training_set.append(np.array([node_total_x[i], node_total_y[i], tag]))
